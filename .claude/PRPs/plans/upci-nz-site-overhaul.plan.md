...
| # | Task | Notes / hazards |
|---|---|---|
| T45 ⏸ | Rework **B's** hero to the "ten congregations, named" motif (D11) — list above the fold under three region headings; demote search to a filter over the visible list | ⏸ **Genuinely gated.** Written against Direction B, which the client rejected on sight. There is no "B hero" left to rework — this task cannot be completed, only **rewritten** against whichever of D1/D2/D3 is chosen. Note D2 (search-led) already carries this motif natively |
...
| T51 ⏸ | **Spike: the `cards` block under B.** 41 items across 7 pages; B's stated position is anti-card | ⏸ **Genuinely gated.** "Anti-card" is Direction B's position, and B is rejected — the spike tests a constraint that no longer binds. D1/D2/D3 each take a different stance on cards, so the answer changes with the choice. The blast radius stands regardless: whatever `cards` becomes changes **7 pages at once**, so this must be proven on `about/leadership` and `apostolic-bible-college` before it lands |
