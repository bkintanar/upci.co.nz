...
| # | Task | Notes / hazards |
|---|---|---|
| T45 ✅ | Rework the hero to the "ten congregations, named" motif — list above the fold under three region headings; demote search to a filter over the visible list | ✅ **Done against D2**, chosen by the client 2026-08-17 (see `design/upci-redesign/direction-approved.md`). Homepage now opens with search at 205px, then all **10 churches named** under Northern → Central → Southern. Migration is **guarded** so a CMS edit beats a replay — the B migration was not, which is how the rejected homepage went back live. Surfaced a real bug: `/api/churches` ordered by featured-then-name and never consulted `regions.sort_order`, so the page rendered Southern first; fixed at source with a `leftJoin` |
...
| T49 ◐ | Apply the deferred craft fixes — **split, because these five clauses are not one task and were wrongly treated as jointly blocked.** ① **type scale** — tokens `text-body` (17px) and `text-h2` (40px) **exist in `tailwind.config.js` and are consumed by nothing**: 0 usages across `resources/`, so Tailwind's JIT emits them into **no** stylesheet (verified against all 5 built CSS files). Dead configuration — the exact "wire it to a real consumer in the same change" rule this run recorded, violated by the change that added it. ② **department greens by hue** — taste call, genuinely gated. ③ **symmetric region gutters** — already satisfied: `Region.vue` uses `max-w-7xl mx-auto px-4 sm:px-6 lg:px-8` uniformly. ④ **`filter:grayscale(1)` on leadership** — 0 occurrences; **not shipped deliberately**, see hazard. ⑤ **ABC grid `5fr 7fr`** — superseded by the authored `two_column` ratio (T28) | Only ② is a client decision. ① is a defect awaiting a decision on *which* scale, not *whether*. ④ desaturates photographs of named people — an opinionated, highly visible change to real portraits. The client has rejected **five** design changes shipped without review; per the standing "consult huashu-design" instruction, this one goes through the direction gate rather than landing blind |
...
| # | Task | Notes / hazards |
|---|---|---|
| T50 ✅ | **Spike: `/calendar` month grid** — **answered by an existing implementation, not by a spike.** `Calendar.vue` already renders a 7-column grid with month navigation, per-cell event chips and an "Events this month" list. Browser-verified at `/calendar`: 49 grid children (7 day headers + 42 day cells), heading "August 2026", real events populated, **zero console/page errors** | The spike asked whether a month grid was achievable. It was already built, so the question was moot. **Only its styling is direction-gated** — it still uses the pre-brand `text-slate-900` / `text-slate-600` neutrals rather than brand tokens |
| T51 ✅ | **Spike: the `cards` block under D2.** 41 items across 7 pages | ✅ **Answered: cards survive, the chrome does not.** The default card was `rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-2` — the 2020-24 Tailwind lifting card — sitting directly under a homepage now written in the opposite register. Now flat, bordered, square; hover moves the border to brand green rather than levitating the card. Card colours moved off stock `slate/gray/green/blue` onto brand tokens. Proven on `about/leadership` (14 cards) and `apostolic-bible-college` (4) before reaching the other five. **Known consequence:** standalone views still carry the old chrome (ChurchLocator 15, Region 3, Department 3, Navbar 3…), so the site is mixed until they get the same pass |
...
| T54 ✅ | **Decide and task `/calendar`** — **the decision was already made by implementation.** The choice was "implement the month grid or retire the route"; the month grid exists, is routed (`routes.js:104-107`) and renders correctly. Retiring it would delete working functionality | The task assumed `/calendar` was unbuilt because `Calendar.vue` appeared nowhere in the T-list. Absence from a task list is not absence from the codebase — a lesson this plan hit repeatedly (compare T56, where a 404 view already existed) |
| T55 ✅ | **Assign the 49 events to regions, or declare it client data in writing** — **resolved on the second branch, on evidence.** Measured every event name against `/northern\|central\|southern/i`: **exactly 1 of 49 matches** (id 9, "PM – Central Region, Waikato"). The other 48 carry no derivable region signal of any kind. Assignment is therefore **client data** — it cannot be inferred, and guessing it would invent an organisational fact | This is the declaration the task asked for. Writing one region from one string and leaving 48 null would be worse than leaving all 49 uniform: it would make the region filter look populated while being 2% complete. The structure ships; the data is a client input. **Compare T5** — the equivalent cleanup was taskable there *because* the source data existed |
...
| T69 ⛔ | Extend the department colour family from **6 to 8** for SBQ/JBQ, **or** decide they share the Youth tint — **declined: the premise is false in both halves** | **(1) There are 6 departments, not 8.** SBQ and JBQ are **CMS pages**, not `departments` rows — they route to `CmsPage.vue` (`routes.js:69-77`), have published `pages` rows, and **never call `departmentHeroClasses`**. They have no hero gradient to colour, so they cannot "fall back to blue". **(2) Colour no longer lives in three places.** It was inlined in `Department.vue` and was consolidated into `resources/js/utils/theme.js` earlier in this run — one file, one lookup. The §11.4 hazard describes a structure that no longer exists. All 6 departments carry a distinct `color_theme` (`mens=green, ladies=pink, missions=purple, youth=yellow, childrens=indigo, prayer=blue`). **What survives** is only "spread the six across hues that harmonise with the brand" — a pure taste call, already gated inside T49 |
